...
from .utils import parse_xlsx_snippets # Assuming parse_xlsx_snippets is in core/utils.py
...
class Pipeline:
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        try:
            ...
            pyannote_model_source = config.get("pyannote_diarization_model", "pyannote/speaker-diarization")
            hf_token = config.get("hf_token")
            if not hf_token:
                 log_warning("Hugging Face token is not set. Pyannote diarization may fail.")

            self.diarizer = PyannotePipeline.from_pretrained(
                 pyannote_model_source,
                 use_auth_token=hf_token
            )
            log_info(f"Pyannote diarization model '{pyannote_model_source}' loaded successfully.")
    ...
    def _run_ffprobe_duration_check(self, audio_path: Path) -> bool:
        ...
        try:
            ...
            if duration < min_dur:
                log_warning(f"Audio too short ({duration:.1f}s < {min_dur}s) for optimal diarization.")
                return False
            ...
            log_warning("Duration check failed, proceeding but diarization quality may be affected.")
            return False
        return True
    ...
    def _find_whisperx_output(self, out_dir: Path, audio_path: Path) -> Path:
        ...
        if expected.exists():
            ...
            return expected
        log_warning(f"Expected WhisperX output {expected.name} not found; searching directory {out_dir}...")
        ...
        if found_json_files:
            if len(found_json_files) > 1:
                 log_warning(f"Multiple potential WhisperX JSON files found in {out_dir}. Using the first one: {found_json_files[0].name}")
    ...
    def _match_snippets_to_speakers(
        ...
        """
        log_info(f"Attempting to match {len(speaker_snippet_map)} snippets to speakers using rapidfuzz.fuzz.partial_ratio after normalization...")
        ...
        for name, snippet in speaker_snippet_map.items():
            if not snippet or len(snippet) < 5:
                log_warning(f"Ignoring short or empty snippet for speaker '{name}'. Snippet: '{snippet}')")
                continue
            ...
            log_info(f"Raw original snippet string repr: {repr(snippet)}") # Log original snippet repr
            ...
            for i, blk in enumerate(blocks):
                ...
                if ratio > best_match_for_snippet["score"]:
                    ...
                    best_match_for_snippet["block_text_snippet"] = txt[:100] + "..." if len(txt) > 100 else txt
                ...
                if ratio >= thresh:
                    ...
                    if ratio > best_match_scores.get(sid, -1.0):
                         ...
                         log_info(f"Match FOUND for snippet '{name}' (score {ratio:.2f}) against speaker '{sid}'.")
            ...
            if not match_found_above_threshold:
                 log_info(f"No match >= threshold ({thresh:.2f}) found for snippet '{name}'. Best match found across all blocks was score {best_match_for_snippet['score']:.2f} against speaker '{best_match_for_snippet['speaker_id']}'. Block text snippet: '{best_match_for_snippet['block_text_snippet']}'")


        log_info(f"Final snippet mapping (WhisperX ID -> User Name): {speaker_id_to_name_mapping}")
        return speaker_id_to_name_mapping


    # MODIFIED: Revised logic for generating previews based on original IDs and mapping
    # (No changes to this method needed for fuzzywuzzy, it uses the mapping from _match_snippets_to_speakers)
    def _get_speaker_previews(
        self,
        segments: SegmentsList, # Receive segments (should have original SPEAKER_XX IDs and analysis results)
        initial_mapping: Dict[str, str], # Receive the initial mapping (WhisperX_ID -> User_Name)
    ) -> SpeakerPreviewsList:
        """
        Create UI worksheet of original speaker IDs + preview text + suggested label.
        Uses initial_mapping to suggest labels for matched IDs.
        """
        previews: SpeakerPreviewsList = []
        ...
        for seg in segments:
            ...
            if sid not in speaker_previews and text:
                speaker_previews[sid] = text[:80] + "..." if len(text) > 80 else text


        all_speaker_ids_in_segments = sorted(list(speaker_previews.keys()))

        for sid in all_speaker_ids_in_segments:
            if sid == "unknown" and sid not in initial_mapping:
                 if not speaker_previews.get(sid):
                     continue

            # Use the mapped name if the original ID is in the initial_mapping, otherwise use an empty string
            suggested_label = initial_mapping.get(sid, "")

            preview_text = speaker_previews.get(sid, "[no text preview]")

            previews.append({
                "Speaker ID": sid,
                "Dialogue Preview": preview_text,
                "Enter Label Here": suggested_label
            })
            seen_speaker_ids.add(sid)

        for original_sid, user_name in initial_mapping.items():
            if original_sid not in seen_speaker_ids:
                previews.append({
                    "Speaker ID": original_sid,
                    "Dialogue Preview": "[matched by snippet, no segment text found]",
                    "Enter Label Here": user_name
                })
                seen_speaker_ids.add(original_sid)

        previews.sort(key=lambda x: x.get("Speaker ID", ""))

        log_info(f"Generated {len(previews)} speaker previews for UI based on original speaker IDs.")
        return previews

    def _save_script_transcript(self, segments: SegmentsList, output_dir: Path, suffix: str) -> Optional[Path]:
    ...
    def _calculate_emotion_summary(
        ...
        for seg in segments:
            ...
            if include_timeline:
                timeline_entry: Dict[str, Any] = {"time": seg.get("start", 0.0), "emotion": emo}
                if 'fused_emotion_confidence' in seg:
                     timeline_entry['fused_emotion_confidence'] = seg['fused_emotion_confidence']
                if 'significant_text_emotions' in seg:
                     timeline_entry['significant_text_emotions'] = seg['significant_text_emotions']
        ...
        for spk, emos in speaker_stats.items():
            ...
            dominant = Counter(filterable_emotions).most_common(1)[0][0] if filterable_emotions else "unknown" # Fallback to unknown
            ...
            if include_timeline:
                 entry["emotion_timeline"] = sorted(timeline[spk], key=lambda x: x.get("time", 0.0))
    ...
    def _save_emotion_summary(
        ...
        try:
            json_path.write_text(json.dumps(stats_serializable, indent=2, ensure_ascii=False), encoding="utf-8")
        ...
        all_emotion_types_in_counts = sorted(list(set(
            e for count_dict in [data.get("emotion_counts", {}) for data in stats.values()]
            for e in count_dict.keys()
        )))

        standard_headers = ["speaker", "total_segments", "emotion_transitions", "dominant_emotion", "emotion_volatility", "emotion_score_mean"]
        final_headers = standard_headers + [h for h in all_emotion_types_in_counts if h not in standard_headers]
        ...
        try:
            with open(csv_path, "w", newline="", encoding="utf-8") as cf:
                ...
                for spk, data in stats.items():
                    row = {"speaker": spk}
                    row.update({k: data.get(k, '') for k in standard_headers if k != "speaker"})
    ...
    def _create_final_zip(
        ...
        try:
            ...
            log_error(f"Failed to create ZIP file {zip_path}: {e}\n{traceback.format_exc()}")
            if temp_zip_path.exists():
                try:
                    temp_zip_path.unlink()
                    log_info(f"Cleaned up temporary zip file: {temp_zip_path}")
                except Exception as unlink_e:
                    log_warning(f"Failed to clean up temporary zip file {temp_zip_path}: {unlink_e}")
    ...
    def _process_batch_item(
        ...
        audio_path_in_work_dir = None # To store the path to the copied/downloaded WAV in work_path
        ...
        try:
            ...
            audio_path = self._prepare_audio_input(input_source, work_path, log_file_handle, job_id)
            audio_path_in_work_dir = audio_path # This is the path within the job's temp directory
            ...
            if not duration_check_ok:
                 log_warning(f"[{job_id}] Audio duration check issues or ffprobe not available.")

            # --- Run WhisperX ---
            status_message = f"[{job_id}] Running WhisperX for transcription and diarization..."
            log_info(status_message)
            log_file_handle.write(f"{status_message}\n")
            log_file_handle.flush()
            self.transcription.run_whisperx(str(audio_path), str(output_temp_dir), log_file_handle, job_id)
            ...
            segments = self.transcription.convert_json_to_structured(str(whisperx_json_path)) # Segments have original SPEAKER_XX IDs
            ...
            segments = self.mm.analyze(segments, str(audio_path), input_source) # Assuming analyze can handle audio_path
            ...
            snippet_mapping = self._match_snippets_to_speakers(segments, speaker_snippet_map or {})
    ...
    def _finalize_batch_item(
        ...
        segments: SegmentsList,
        snippet_mapping: Dict[str, str],
        audio_path_in_work_dir: Optional[Path], # Path to the WAV file in the item work directory
        ...
        def item_log(level, message):
            ...
            if log_file_handle:
                 try:
                     log_file_handle.write(f"{datetime.utcnow().strftime('%Y-%m-%d %H:%M:%S')} - {level.upper()} - {full_message}\n")
                     log_file_handle.flush()
                 except Exception as log_e:
                     log_error(f"[{item_identifier}] Failed to write to batch log file: {log_e} - Message: {message}")
                     print(f"WARN: Failed to write to batch log file: {log_e} - Message: {message}") # Fallback print
            # Also log to main application logger
            if level == 'info': log_info(full_message)
            elif level == 'warning': log_warning(full_message)
            elif level == 'error': log_error(full_message)


        item_log('info', f"Starting finalization for {item_identifier} in: {item_work_path}")
        status_msg = f"[{item_identifier}] Starting finalization..."

        try:
            if not segments:
                 status_msg = f"[{item_identifier}] ERROR: No segments found for finalization."
            ...
            for seg in segments:
                 original_speaker_id_in_segment = seg.get("speaker") # This is the original SPEAKER_XX ID
                 original_speaker_id_str = str(original_speaker_id_in_segment) if original_speaker_id_in_segment is not None else "unknown"

                 # Apply the snippet-matched label if a mapping exists for this original SPEAKER_XX ID
                 if original_speaker_id_str in snippet_mapping:
                      user_label = snippet_mapping[original_speaker_id_str]
                      if user_label: # Only relabel if the snippet mapping provided a non-empty label
                         seg["speaker"] = user_label
                         segments_relabeled_count += 1
                      else:
                          # If snippet mapping was empty string, keep original SPEAKER_XX ID
                          seg["speaker"] = original_speaker_id_str # Explicitly set to original ID if empty label provided
                 # If original_speaker_id_str is NOT in snippet_mapping, it remains its original SPEAKER_XX ID

            item_log('info', f"Applied snippet labels to {segments_relabeled_count} segments.")
            ...
            output_temp_dir = item_work_path / "output" # Save within the item's output subdir
            output_temp_dir.mkdir(exist_ok=True) # Ensure output subdir exists
            final_json_name = f"{item_identifier}_{Path(FINAL_STRUCTURED_TRANSCRIPT_NAME).name}"
            ...
            item_log('info', f"Final relabeled structured transcript saved to: {final_json_path}")
            ...
            script_transcript_path = self._save_script_transcript(segments, output_temp_dir, item_identifier)
            if script_transcript_path:
                 item_log('info', f"Script transcript saved to: {script_transcript_path}")
            ...
            csv_path, json_summary_path = self._save_emotion_summary(stats, output_temp_dir, item_identifier)
            if csv_path and json_summary_path:
                item_log('info', f"Emotion summary saved to: {csv_path}, {json_summary_path}")
            else:
                 item_log('warning', "Failed to save all emotion summary files for this item.")
            ...
            try:
                ...
                item_log('warning', f"Plot generation failed for {item_identifier}: {p}")
                plots = []
    ...
    def process_batch_xlsx(
        ...
        batch_work_path = base_temp_dir / batch_job_id # Main working directory for the batch
        batch_log_path = batch_work_path / LOG_FILE_NAME # Main log file for the batch process
        log_file_handle = None
        processed_item_identifiers: List[str] = [] # To track which items were processed for cleanup
        ...
        try:
            ...
            def batch_log(level, message):
                 ...
                 if log_file_handle:
                     try:
                          ...
                          log_error(f"[{batch_job_id}] Failed to write to batch log file: {log_e} - Message: {message}")
                          print(f"WARN: Failed to write to batch log file: {log_e} - Message: {message}") # Fallback print
                 # Also log to main application logger
                 if level == 'info': log_info(full_message)
                 elif level == 'warning': log_warning(full_message)
            ...
            if url_col not in df.columns:
                 raise ValueError(f"Required column '{url_col}' not found in the XLSX file.")

            batch_log('info', f"Using URL column: '{url_col}' and Snippet column: '{snippet_col}'")

            for index, row in df.iterrows():
                # Using f"item_{index+1}" as item_identifier
                item_identifier = f"item_{index+1}"
                ...
                if not isinstance(youtube_url_raw, str) or not youtube_url_raw.strip():
                    batch_log('warning', f"[{item_identifier}] Skipping row {index+1} due to missing or invalid YouTube URL.")
                    results.append(f"[{item_identifier}] Skipped: Missing/Invalid YouTube URL.")
                    continue

                processed_item_identifiers.append(item_identifier) # Add to list ONLY if URL is valid
                ...
                speaker_snippet_map = parse_xlsx_snippets(snippets_string) # Call function from utils
                batch_log('info', f"[{item_identifier}] Parsed snippets: {speaker_snippet_map}")
                ...
                segments, audio_path_in_work_dir, snippet_mapping = self._process_batch_item(
                     youtube_url,
                     speaker_snippet_map,
                ...
                if segments is not None:
                    ...
                    results.append(f"[{item_identifier}] Failed during processing step.")
            ...
            try:
                with zipfile.ZipFile(str(temp_master_zip_path), "w", zipfile.ZIP_DEFLATED) as zf:
                    ...
                    if batch_log_path.exists():
                        ...
                        batch_log('info', f"Added batch log file to master zip: {batch_log_path.name}")

                    # Iterate through each item's working directory and add its contents to the zip
                    for item_id in processed_item_identifiers: # Only iterate through successfully identified items
                ...
                batch_log('info', f"Successfully created master ZIP: {master_zip_location}")
                ...
                batch_results_summary = "Batch Results Summary:\n" + "\n".join(results) + f"\n\nMaster ZIP: {master_zip_path_str}"
                batch_log('info', batch_status_message)
             ...
             batch_status_message = f"[{batch_job_id}] ERROR: Batch input file not found at {xlsx_filepath}"
            ...
            if batch_work_path.exists():
                 batch_log('info', f"Cleaning up batch temporary directory: {batch_work_path}")
                 try:
                     shutil.rmtree(batch_work_path)
                     batch_log('info', f"Batch temporary directory removed: {batch_work_path}")
                 except OSError as e:
                     batch_log('warning', f"Failed to remove batch temporary directory {batch_work_path}: {e}")
