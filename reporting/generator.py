...
from utils.subprocess import (
    ...
    SubprocessError,
)  # Use the project's subprocess runner
...
def _extract_audio_snippet(
    ...
    cmd = [
        ...
        "libmp3lame",
        "-q:a",
        "5",  # Quality preset (0-9, lower is better)
    ...
    try:
        ...
        )  # Use capture_output to hide ffmpeg verbose logs unless error
        if output_path.exists() and output_path.stat().st_size > 0:
            logger.info(f"Successfully created snippet: {output_path.name}")
            return output_path
        else:
            logger.error(
                f"ffmpeg ran but output snippet {output_path.name} is missing or empty."
            )
            return None
    except SubprocessError as e:
        logger.error(
            f"ffmpeg failed to extract snippet {output_path.name}: {e}", exc_info=False
        )
        ...
        logger.error(
            f"Unexpected error extracting snippet {output_path.name}: {e}",
            exc_info=True,
        )
        return None
...
def generate_all(
    ...
    *,  # Make new args keyword-only for clarity
    ...
    | None = None,  # List of speakers needing snippets/transcripts
) -> Tuple[
    ...
    can_generate_snippets = (
        cfg.enable_interactive_labeling
        and eligible_speakers
        and wav_path
        and wav_path.exists()
        and shutil.which("ffmpeg")  # Check if ffmpeg command exists
    )
    if cfg.enable_interactive_labeling and not can_generate_snippets:
        logger.warning(
            "Interactive labeling enabled, but cannot generate speaker snippets."
        )
        if not eligible_speakers:
            logger.warning("- No eligible speakers identified.")
        if not wav_path or not wav_path.exists():
            logger.warning(f"- Source WAV path invalid or missing: {wav_path}")
    ...
    if can_generate_snippets:
        logger.info(
            f"Generating snippets/transcripts for {len(eligible_speakers)} speakers..."
        )
        ...
        for speaker_id in eligible_speakers:
            ...
            if not spk_segments:
                logger.warning(
                    f"Speaker {speaker_id} is eligible but has no segments? Skipping snippet gen."
                )
                continue
            ...
            if not blocks:
                logger.warning(
                    f"Could not group segments into blocks for speaker {speaker_id}. Skipping snippet gen."
                )
                continue

            longest_block = max(
                blocks, key=lambda b: b.get("end", 0) - b.get("start", 0)
            )
            ...
            if start_time is None:
                logger.warning(
                    f"Longest block for speaker {speaker_id} has no start time. Skipping snippet gen."
                )
                continue
            ...
            if extracted_path:
                speaker_labeling_data[speaker_id] = {
                    "audio_snippet_path": str(
                        extracted_path
                    ),  # Store as string for JSON etc.
                    "example_transcript": example_transcript.strip(),
                }
            else:
                logger.warning(
                    f"Failed to generate audio snippet for speaker {speaker_id}."
                )
    ...
    if cfg.include_plots:
        ...
        try:
            ...
            logger.error(f"Failed to generate trajectory plot: {e}", exc_info=True)
            plot_errors = True
        try:
            ...
            logger.error(f"Failed to generate distribution plot: {e}", exc_info=True)
            plot_errors = True
        try:
            # Ensure stats['per_speaker'] exists before passing
            if stats and "per_speaker" in stats:
                vol = pltg.plot_volatility(
                    stats["per_speaker"], plots_dir / "volatility.png"
                )
                if vol:
                ...
                logger.warning(
                    "Skipping volatility plot: 'per_speaker' data not found in summary stats."
                )
        except Exception as e:
            logger.error(f"Failed to generate volatility plot: {e}", exc_info=True)
            plot_errors = True
        try:
            ...
            logger.error(f"Failed to generate intensity plot: {e}", exc_info=True)
            plot_errors = True
